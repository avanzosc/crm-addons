...
class CrmPhonecall(models.Model):
    ...
    phonecall_time_ids = fields.One2many(
        string='Phone calls', comodel_name='crm.phonecall.time',
    ...
    commercial_partner_id = fields.Many2one(
        string='Commercial partner', comodel_name='res.partner',
        ...
        if self.partner_phone:
            ...
            if len(partners) > 1:
                self.commercial_partner_id = (
                    partners[0].commercial_partner_id.id)
    ...
    def _compute_show_init_call(self):
        for inbound in self:
            if not inbound.phonecall_time_ids:
                ...
                found = inbound.phonecall_time_ids.filtered(
