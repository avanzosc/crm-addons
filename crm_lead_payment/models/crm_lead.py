            ...
            if line.partner_id:
                if line.partner_id.company_type == "company":
                    ...
                    if line.partner_id.child_ids:
                        possible_shippings += line.partner_id.child_ids.filtered(
                    ...
                    if line.partner_company_id.parent_id:
                        ...
                        if line.partner_company_id.parent_id.child_ids:
                            possible_shippings += (
                                line.partner_company_id.parent_id.child_ids.filtered(
                                    lambda c: c.type == "delivery"
                                )
                            )
        ...
        for line in self:
            ...
            if line.partner_id:
                if line.partner_id.company_type == "company":
                    ...
                    if line.partner_company_id.child_ids:
                        possible_invoice += line.partner_company_id.child_ids.filtered(
                            lambda c: c.type == "invoice"
                        )
                    if line.partner_company_id.parent_id:
