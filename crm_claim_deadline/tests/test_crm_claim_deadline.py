...
class TestCrmClaimDeadline(common.TransactionCase):
    def setUp(self):
        ...
        company2_vals = self.company.copy_data(
            default={
                "name": "New Company",
    ...
    def test_crm_claim_deadline(self):
        vals = {"name": "Testing module", "partner_id": self.partner.id}
        claim = self.claim_model.create(vals)
        self.assertNotEqual(claim.date_deadline, False, "Claim Deadline no generated")
