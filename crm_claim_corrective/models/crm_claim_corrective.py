...
class CrmClaimCorrective(models.Model):
    ...
    partner_id = fields.Many2one(
        comodel_name='res.partner', related="claim_id.partner_id",
...
class CrmClaimCorrectiveAction(models.Model):
    ...
    corrective_id = fields.Many2one(comodel_name='crm.claim.corrective',
                                    string='Corrective info')
    name = fields.Char(string='Name', required=True)
    sequence = fields.Integer(string='Sequence')
    claim_id = fields.Many2one(
        comodel_name='crm.claim', related="corrective_id.claim_id",
        string="Claim", store=True, readonly=True)
